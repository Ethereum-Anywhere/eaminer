...
bool CUDAMiner::initDevice() {
    cextr << "Using Pci " << m_deviceDescriptor.uniqueId << ": " << m_deviceDescriptor.boardName
          << " (Compute " + m_deviceDescriptor.cuCompute + ") Memory : " << dev::getFormattedMemory((double) m_deviceDescriptor.totalMemory);
...
bool CUDAMiner::initEpoch() {
    ...
    size_t RequiredTotalMemory(m_epochContext.dagSize + m_epochContext.lightSize + m_deviceDescriptor.cuStreamSize * sizeof(Search_results));
    ReportGPUMemoryRequired(m_epochContext.lightSize, m_epochContext.dagSize, m_deviceDescriptor.cuStreamSize * sizeof(Search_results));
    try {
        hash128_t* dag;
        hash64_t* light;
        ...
        set_constants(dag, m_epochContext.dagNumItems, light, m_epochContext.lightNumItems);   // in ethash_cuda_miner_kernel.cu
...
void CUDAMiner::workLoop() {
    ...
    try {
        while (!shouldStop()) {
            ...
            if (hr >= 1e7) m_block_multiple = uint32_t((hr * CU_TARGET_BATCH_TIME) / (m_deviceDescriptor.cuStreamSize * m_deviceDescriptor.cuBlockSize));
...
void CUDAMiner::kick_miner() {
    ...
    if (!m_done) {
        m_done = true;
        for (unsigned i = 0; i < m_deviceDescriptor.cuStreamSize; i++)
            CUDA_CALL(cudaMemcpyAsync((uint8_t*) m_search_buf[i] + offsetof(Search_results, done), &one, sizeof(one), cudaMemcpyHostToDevice));
    }
}
...
int CUDAMiner::getNumDevices() {
    ...
    if (err == cudaSuccess) return deviceCount;
    ...
    if (err == cudaErrorInsufficientDriver) {
        ...
        if (driverVersion == 0) {
            ...
            cwarn << "Insufficient CUDA driver " << std::to_string(driverVersion);
        }
    } else {
...
void CUDAMiner::enumDevices(minerMap& DevicesCollection) {
    ...
    for (int i = 0; i < numDevices; i++) {
        ...
        try {
            ...
            s << std::setw(4) << std::setfill('0') << std::hex << props.pciDomainID << ':' << std::setw(2) << props.pciBusID << ':' << std::setw(2) << props.pciDeviceID
              << ".0";
            uniqueId = s.str();

            if (DevicesCollection.find(uniqueId) != DevicesCollection.end()) {
                deviceDescriptor = DevicesCollection[uniqueId];
            } else {
                deviceDescriptor = DeviceDescriptor();
            }

            deviceDescriptor.boardName = std::string(props.name);
            ...
            deviceDescriptor.cuCompute = (std::to_string(props.major) + "." + std::to_string(props.minor));
        ...
        } catch (const cuda_runtime_error& _e) { ccrit << _e.what(); }
    }
}
...
void CUDAMiner::search(uint8_t const* header, uint64_t target, uint64_t start_nonce, const dev::eth::WorkPackage& w) {
    ...
    for (uint32_t streamIdx = 0; streamIdx < m_deviceDescriptor.cuStreamSize; streamIdx++, start_nonce += batch_blocks) {
        HostToDevice(m_search_buf[streamIdx], zero3, sizeof(zero3));
        m_hung_miner.store(false);
        run_ethash_search(m_block_multiple, m_deviceDescriptor.cuBlockSize, m_streams[streamIdx], m_search_buf[streamIdx], start_nonce);
    }
    m_done = false;
    m_doneMutex.unlock();
    ...
    while (streams_bsy) {
        ...
        for (uint32_t streamIdx = 0; streamIdx < m_deviceDescriptor.cuStreamSize; streamIdx++, start_nonce += batch_blocks) {
            ...
            if (!(streams_bsy & stream_mask)) continue;

            cudaStream_t stream(m_streams[streamIdx]);
            auto* buffer = (uint8_t*) m_search_buf[streamIdx];
            ...
            if (m_done) {
                ...
                m_hung_miner.store(false);
                run_ethash_search(m_block_multiple, m_deviceDescriptor.cuBlockSize, stream, (Search_results*) buffer, start_nonce);
            }

...
#define HOST_INLINE static __host__ __forceinline__

//#undef __CUDA_ARCH__
//#define __CUDA_ARCH__ 10
...
#if __CUDA_ARCH__ < 320   // Kepler (Compute 3.0)
    return ((x) << (n)) | ((x) >> (32 - (n)));
#else   // Kepler (Compute 3.5, 5.0)
...
// Endian swap for 32 Bit Types
#ifdef __CUDA_ARCH__ /* device */
DEV_INLINE uint32_t cuda_swab32(const uint32_t x) { return __byte_perm(x, x, 0x0123); }
#else /* host */
HOST_INLINE uint32_t cuda_swab32(const uint32_t x) {
...
#else /* host */
HOST_INLINE uint64_t cuda_swab64(const uint64_t x) {
...
#if USE_XOR_ASM_OPTS   // device asm for whirpool
    uint64_t result;
...
DEV_INLINE uint64_t xor8(const uint64_t a, const uint64_t b, const uint64_t c, const uint64_t d, const uint64_t e, const uint64_t f, const uint64_t g, const uint64_t h) {
#if USE_XOR_ASM_OPTS   // device asm for whirpool
    uint64_t result;
...
#else /* host */
HOST_INLINE uint64_t ROTR64(const uint64_t x, const int offset) { return (((x) >> (offset)) | ((x) << (64 - (offset)))); }
#endif
...
#else /* host */
HOST_INLINE uint64_t ROTL64(const uint64_t x, const int offset) { return (x << offset) | (x >> (64 - offset)); }
#endif
...
DEV_INLINE void LOHI(uint32_t& lo, uint32_t& hi, uint64_t x) { asm("mov.b64 {%0,%1},%2; \n\t" : "=r"(lo), "=r"(hi) : "l"(x)); }
...
DEV_INLINE void devectorize2(const uint4& inn, uint2& x, uint2& y) {
    x.x = inn.x;
    x.y = inn.y;
    y.x = inn.z;
    y.y = inn.w;
...
DEV_INLINE uint4 vectorize2(const uint2& x, const uint2& y) {
    uint4 result;
    result.x = x.x;
    result.y = x.y;
    result.z = y.x;
...
DEV_INLINE uint4 vectorize2(const uint2& x) {
    uint4 result;
    result.x = x.x;
    result.y = x.y;
    result.z = x.x;
...
DEV_INLINE uint2 operator^(uint2 a, uint32_t b) { return make_uint2(a.x ^ b, a.y); }
DEV_INLINE uint2 operator^(uint2 a, uint2 b) { return make_uint2(a.x ^ b.x, a.y ^ b.y); }
DEV_INLINE uint2 operator&(uint2 a, uint2 b) { return make_uint2(a.x & b.x, a.y & b.y); }
DEV_INLINE uint2 operator|(uint2 a, uint2 b) { return make_uint2(a.x | b.x, a.y | b.y); }
DEV_INLINE uint2 operator~(uint2 a) { return make_uint2(~a.x, ~a.y); }
DEV_INLINE void operator^=(uint2& a, uint2 b) { a = a ^ b; }
DEV_INLINE uint2 operator+(uint2 a, uint2 b) {
    uint2 result;
    asm("{\n\t"
        "add.cc.u32 %0,%2,%4; \n\t"
        "addc.u32 %1,%3,%5;   \n\t"
...
DEV_INLINE uint2 operator+(uint2 a, uint32_t b) {
    uint2 result;
    asm("{\n\t"
        "add.cc.u32 %0,%2,%4; \n\t"
        "addc.u32 %1,%3,%5;   \n\t"
...
DEV_INLINE uint4 operator^(uint4 a, uint4 b) { return make_uint4(a.x ^ b.x, a.y ^ b.y, a.z ^ b.z, a.w ^ b.w); }
DEV_INLINE uint4 operator&(uint4 a, uint4 b) { return make_uint4(a.x & b.x, a.y & b.y, a.z & b.z, a.w & b.w); }
DEV_INLINE uint4 operator|(uint4 a, uint4 b) { return make_uint4(a.x | b.x, a.y | b.y, a.z | b.z, a.w | b.w); }
DEV_INLINE uint4 operator~(uint4 a) { return make_uint4(~a.x, ~a.y, ~a.z, ~a.w); }
DEV_INLINE void operator^=(uint4& a, uint4 b) { a = a ^ b; }
DEV_INLINE uint4 operator^(uint4 a, uint2 b) { return make_uint4(a.x ^ b.x, a.y ^ b.y, a.z ^ b.x, a.w ^ b.y); }
...
DEV_INLINE uint2 SHL2(uint2 a, int offset) {
...
DEV_INLINE uint2 SHR2(uint2 a, int offset) {
#if __CUDA_ARCH__ > 300
    uint2 result;
    if (offset < 32) {
        asm("{\n\t"
...
DEV_INLINE uint64_t devectorizeswap(uint2 v) { return MAKE_ULONGLONG(cuda_swab32(v.y), cuda_swab32(v.x)); }
DEV_INLINE uint2 vectorizeswap(uint64_t v) {
    uint2 result;
...
DEV_INLINE uint4 add4(uint4 a, uint4 b) {
    uint4 result;
    asm("{\n\t"
        "add.cc.u32           %0,%4,%8;  \n\t"
        "addc.u32             %1,%5,%9;  \n\t"
...
DEV_INLINE uint4 madd4(uint4 a, uint4 b) {
    uint4 result;
    asm("{\n\t"
...
DEV_INLINE ulonglong2 madd4long(ulonglong2 a, ulonglong2 b) {
    ulonglong2 result;
    asm("{\n\t"
...
DEV_INLINE void madd4long2(ulonglong2& a, ulonglong2 b) {
    asm("{\n\t"
        ".reg .u32 a0,a1,a2,a3,b0,b1,b2,b3;\n\t"
        "mov.b64 {a0,a1}, %0;\n\t"
        "mov.b64 {a2,a3}, %1;\n\t"
...
DEV_INLINE void __prefetch_global_l1(const void* const ptr) { asm("prefetch.global.L1 [%0];" : : "l"(ptr)); }

DEV_INLINE void __prefetch_global_l2(const void* const ptr) { asm("prefetch.global.L2 [%0];" : : "l"(ptr)); }

DEV_INLINE void __prefetch_constant_l1(const void* const ptr) { asm("prefetchu.L1 [%0];" : : "l"(ptr)); }

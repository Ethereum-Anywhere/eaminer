...
# Run docker: docker build -t eaminer . && docker run --gpus all -e POOL="stratum+tcp://ikru.eth:x@us-east.ethash-hub.miningpoolhub.com:20535" eaminer 
FROM nvidia/cuda:11.3.0-base-ubuntu18.04
ENV POOL="Pool connection"
WORKDIR /app
COPY --from=build /app/ ./

CMD nvidia-smi && ./build/eaminer/eaminer -U -P "${POOL}"

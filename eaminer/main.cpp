...
bool g_seqDAG = false;
bool g_exitOnError = false;   // Whether miner should exit on mining threads errors
mutex g_seqDAGMutex;
...
static void headers(vector<string>& h, bool color) {
    ...
    auto* bi = eaminer_get_buildinfo();
    stringstream ss;
    ss << yellow << "eaminer " << bi->project_version << " Run Ethereum Anywhere (Heterogeneous Edition)";
    h.push_back(ss.str());

    ss.str("");
    ss << white << "Licensed under the terms of the GNU General Public License Version 3";
    h.push_back(ss.str());
...
static void on_help_module(const string& m) {
    ...
    if (find(modules.begin(), modules.end(), m) != modules.end()) return;
...
static void on_nonce(const string& n) {
    for (const auto& c: n) {
        if ((c < '0' || c > '9') && (c < 'a' || c > 'f') && (c < 'A' || c > 'F')) { throw boost::program_options::error("The --nonce value must be a hex string"); }
    }
}

static void on_verbosity(unsigned u) {
    if (u < LOG_NEXT) return;
...
static void on_cu_block_size(unsigned b) {
    if (b == 32 || b == 64 || b == 128 || b == 256) return;
...
public:
    ...
    MinerCLI() : m_cliDisplayTimer(g_io_service), m_io_strand(g_io_service) {
        ...
        m_cliDisplayTimer.async_wait(m_io_strand.wrap(boost::bind(&MinerCLI::cliDisplayInterval_elapsed, this, boost::asio::placeholders::error)));

        // Start io_service in its own thread
        m_io_thread = thread{[ObjectPtr = &g_io_service] { ObjectPtr->run(); }};
...
#    define BACKTRACE_MAX_FRAMES 30
            case SIGSEGV:
                ...
                if (!in_handler) {
                    ...
                    for (j = 0; j < nptrs; j++) cerr << symbols[j] << "\n";
                    free(symbols);
...
#    undef BACKTRACE_MAX_FRAMES
    ...
    static void ParseBind(const string& inaddr, string& outaddr, int& outport, bool advertise_negative_port) {
        ...
        if (regex_match(inaddr, matches, pattern)) {
            ...
            if (ec) throw invalid_argument("Invalid Ip Address");
            ...
            if (advertise_negative_port) {
                ...
                if (outport < 1 || outport > 65535) throw invalid_argument("Invalid port number. Allowed non zero values in range [1 .. 65535]");
            }
        } else {
    ...
    bool validateArgs(int argc, char** argv) {
        ...
        misc.add_options()
            ("verbosity,v", value<unsigned>()->default_value(0)->notifier(on_verbosity),
        ...
        try {
            ...
            if (vm.count("config")) {
                ...
                ss << ifs.rdbuf();
                vector<string> args;
                std::string rdbuf_str = ss.str();
                boost::split(args, rdbuf_str, boost::is_any_of(" \n\r\t"), boost::token_compress_on);
            ...
            if (!unknown.empty()) {
                ...
                for (const auto& u: unknown) cout << ' ' << u;
                cout << endl << endl;
                return false;
            }
        ...
        if (vm.count("devices")) {
            for (auto& d: vm["devices"].as<vector<unsigned>>()) m_devices.push_back(d);
        }
        ...
        if (cl_miner) m_minerType = MinerType::CL;
        else if (cuda_miner)
        ...
        if (m_bench) {
            ...
            pools.clear();
            m_PoolSettings.connections.push_back(std::make_shared<URI>("simulation://localhost:0", true));
        } else {
            m_mode = OperationMode::Mining;
        }

        if (!m_shouldListDevices && (m_mode != OperationMode::Simulation)) {
            if (pools.empty()) { throw invalid_argument("At least one pool definition required. See -P argument."); }

            for (size_t i = 0U; i < pools.size(); i++) {
                ...
                try {
                    shared_ptr<URI> uri = std::make_shared<URI>(url);
    ...
    void execute() {
#if ETH_ETHASHCL
        if (m_minerType == MinerType::CL || m_minerType == MinerType::Mixed) CLMiner::enumDevices(m_DevicesCollection);
#endif
#if ETH_ETHASHCUDA
...
#endif
#if ETH_ETHASHCL
            if (m_minerType == MinerType::CL || m_minerType == MinerType::Mixed) cout << setw(5) << "CL   ";
#endif
...
#endif
#if ETH_ETHASHCL
            if (m_minerType == MinerType::CL || m_minerType == MinerType::Mixed) cout << setw(5) << "---- ";
#endif
...
#endif
#if ETH_ETHASHCL
                if (m_minerType == MinerType::CL || m_minerType == MinerType::Mixed) cout << setw(5) << (it->second.clDetected ? "Yes" : "");
#endif
        ...
        if (m_minerType == MinerType::CUDA || m_minerType == MinerType::Mixed)
            for (auto it = m_DevicesCollection.begin(); it != m_DevicesCollection.end(); it++) {
                if (!it->second.cuDetected || it->second.subscriptionType != DeviceSubscriptionTypeEnum::None) continue;
                auto d = (unsigned) distance(m_DevicesCollection.begin(), it);
                if (m_devices.empty() || find(m_devices.begin(), m_devices.end(), d) != m_devices.end()) it->second.subscriptionType = DeviceSubscriptionTypeEnum::Cuda;
            }
#endif
#if ETH_ETHASHCL
        if (m_minerType == MinerType::CL || m_minerType == MinerType::Mixed)
            for (auto it = m_DevicesCollection.begin(); it != m_DevicesCollection.end(); it++) {
                if (!it->second.clDetected || it->second.subscriptionType != DeviceSubscriptionTypeEnum::None) continue;
                unsigned d = (unsigned) distance(m_DevicesCollection.begin(), it);
                if (m_devices.empty() || find(m_devices.begin(), m_devices.end(), d) != m_devices.end()) it->second.subscriptionType = DeviceSubscriptionTypeEnum::OpenCL;
            }
#endif
#if ETH_ETHASHCPU
        if (m_minerType == MinerType::CPU)
            for (auto& it: m_DevicesCollection) { it.second.subscriptionType = DeviceSubscriptionTypeEnum::Cpu; }
#endif
        // Count of subscribed devices
        int subscribedDevices = 0;
        for (auto& it: m_DevicesCollection) {
            if (it.second.subscriptionType != DeviceSubscriptionTypeEnum::None) { subscribedDevices++; }
        }

        // If no OpenCL and/or CUDA devices subscribed then throw error
        if (!subscribedDevices) throw runtime_error("No mining device selected. Aborting ...");
...
private:
    void doMiner() {
        ...
        if (m_mode != OperationMode::Simulation)
            for (const auto& conn: m_PoolSettings.connections) { cnote << "Configured pool " << conn->Host() + ":" + to_string(conn->Port()); }

#if API_CORE

        ApiServer api(m_api_address, m_api_port, m_api_password);
        if (m_api_port) api.start();
#endif
        ...
        while (g_running) g_shouldstop.wait(clilock);
...
#endif
        if (PoolManager::p().isRunning()) PoolManager::p().stop();
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        ...
        cout << "Try 'eaminer --help' to get a list of arguments.";
        return 1;
    }
    ...
    try {
        ...
        try {
            ...
            if (!cli.validateArgs(argc, argv)) return 0;

            if (g_logSyslog) g_logNoColor = true;

            if (!should_list) {
                vector<string> vs;
                headers(vs, !g_logNoColor);
                for (auto& v: vs) cnote << v;
            }

...
static inline void ethash_search_kernel(const sycl::nd_item<1>& item, Search_results* g_output, uint64_t start_nonce, uint64_t d_dag_num_items,
                                        const hash128_t* const d_dag, hash32_t d_header, uint64_t d_target) noexcept {
    ...
    bool r = compute_hash<threads_per_hash_, parallel_hash_, accesses_>(item, start_nonce + gid, d_dag_num_items, d_dag, d_header, d_target);
...
[[nodiscard]] sycl::event run_ethash_search(uint32_t work_groups, uint32_t work_items, sycl::queue& q, Search_results* g_output, uint64_t start_nonce,
                                            uint64_t d_dag_num_items, const hash128_t* const d_dag, hash32_t d_header, uint64_t d_target, sycl::event in_evt) {
    return q.submit([&](sycl::handler& cgh) {
        cgh.depends_on(in_evt);
        cgh.parallel_for(sycl::nd_range<1>(work_groups * work_items, work_items),
                         [=](sycl::nd_item<1> item) /* [[sycl::reqd_sub_group_size(32)]] [[sycl::work_group_size_hint(128)]] */ {
                             ethash_search_kernel(item, g_output, start_nonce, d_dag_num_items, d_dag, d_header, d_target);
                         });
    });
}
...
static inline void ethash_calculate_dag_item(const sycl::nd_item<1>& item, uint32_t start, uint64_t d_dag_num_items, uint32_t d_light_num_items, hash128_t* d_dag,
...
[[nodiscard]] std::vector<sycl::event> ethash_generate_dag(uint64_t dag_size, uint32_t work_groups, uint32_t work_items, sycl::queue& q, uint32_t d_dag_num_items,
                                                           uint32_t d_light_num_items, hash128_t* d_dag, const hash64_t* const d_light, sycl::event evt) {
    const auto work = (int32_t) (dag_size / sizeof(hash64_t));
    const int64_t run = work_groups * work_items;

    std::vector<sycl::event> events{};
    events.reserve((work + run - 1) / run);   // Upper bound

    int64_t base;
    for (base = 0; base <= work - run; base += run) {
        sycl::event submission_evt = q.submit([&](sycl::handler& cgh) {
            cgh.depends_on(evt);
            cgh.parallel_for(sycl::nd_range<1>(run, work_items),
                             [=](sycl::nd_item<1> item) /* [[sycl::reqd_sub_group_size(32)]] [[sycl::work_group_size_hint(128)]] */ {
    ...
    if (base < work) {
        ...
        sycl::event submission_evt = q.submit([&](sycl::handler& cgh) {
            cgh.depends_on(evt);
            cgh.parallel_for(sycl::nd_range<1>(lastGrid * work_items, work_items),
                             [=](sycl::nd_item<1> item) /* [[sycl::reqd_sub_group_size(32)]] [[sycl::work_group_size_hint(128)]] */ {
                                 ethash_calculate_dag_item(item, base, d_dag_num_items, d_light_num_items, d_dag, d_light);
                             });
        });

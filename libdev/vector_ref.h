 ...
 */
template<class _T> class vector_ref {
public:
    using value_type = _T;
    ...
    bool contentsEqual(std::vector<mutable_value_type> const& _c) const {
        if (!m_data || m_count == 0) return _c.empty();
    ...
    template<class _T2> explicit operator vector_ref<_T2>() const { return vector_ref<_T2>(reinterpret_cast<_T2*>(m_data), m_count * sizeof(_T) / sizeof(_T2)); }
    ...
    vector_ref<_T> cropped(size_t _begin) const {
        if (m_data && _begin <= m_count) return vector_ref<_T>(m_data + _begin, m_count - _begin);
        return {};
    }
    ...
    template<class T> bool overlapsWith(vector_ref<T> _t) const {
        void const* f1 = data();
    ...
    void copyTo(vector_ref<typename std::remove_const<_T>::type> _t) const {
        if (overlapsWith(_t)) memmove(_t.data(), m_data, std::min(_t.size(), m_count) * sizeof(_T));
        else

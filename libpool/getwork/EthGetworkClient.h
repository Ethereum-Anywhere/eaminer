...
private:
    ...
    static std::string processError(Json::Value& JRes);
    ...
    std::atomic<bool> m_connecting = {false};   // Whether socket is on first try connect
    std::atomic<bool> m_txPending = {false};    // Whether an async socket operation is pending
